...
namespace Framework.Infrastructure.MessageBus.RabbitMQ
{
    public class PublisherConfirms : IPublisherConfirms
    {
        ...
        public void RegisterCallbacks(IModel channel, Action successCallback, Action failureCallback)
        {
            ...
            {
                throw new Exception("Duplicate NextPublishSeqNo. publish channels are not thread safe. Are you sharing a channel between threads?");
            }
        }
        ...
        private void RunForSequenceNumber(ulong sequenceNumber, Action<CallbackSet> callbackSetAction)
        {
            ...
            {
                throw new Exception("Delivery callback has unrecorded sequence number");
            }
