...
namespace Framework.Infrastructure.MessageBus.RabbitMQ
{
    ...
    public class QueueingConsumerFactory : IConsumerFactory
    {
        ...
        public void HandleMessageDelivery(BasicDeliverEventArgs basicDeliverEventArgs)
        {
            ...
            if (consumerTag == null)
            {
                logger.Error("BasicDeliverEventArgs.ConsumerTag is null");
                return;
            }
            if (!subscriptions.ContainsKey(consumerTag))
            {
                logger.Error(string.Format("No subscription for consumerTag: {0}", consumerTag));
                return;
            }
            ...
            if (!subscriptionInfo.Consumer.IsRunning)
            {
                ...
                logger.Debug(string.Format("Consumer has stopped running. ConsumerTag: {0}", consumerTag));
                return;
            }

            logger.Debug(string.Format("Recieved \n\tRoutingKey: '{0}'\n\tCorrelationId: '{1}'\n\tConsumerTag: '{2}'",
    ...
    public class SubscriptionInfo
    {
        ...
        public MessageBusConsumer Consumer { get; private set; }
